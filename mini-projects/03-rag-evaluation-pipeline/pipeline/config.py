...
class PipelineSettings(BaseSettings):
    # QA generation provider. Embeddings run locally via sentence-transformers
    # (no embedding API), so only a chat key is needed. Anthropic is the default.
    qa_provider: str = "anthropic"
    anthropic_api_key: str = ""
    qa_model_name: str = "claude-haiku-4-5-20251001"
    qa_max_tokens: int = 1024

    # Optional alternative QA providers (used only if qa_provider points to them).
    openai_api_key: str = ""
...
# OpenAI embedding models -- the two-model quality axis from the plan.
# Swap in local sentence-transformers names (e.g. "sentence-transformers/...")
# for a free, offline embedding axis; embedder.py dispatches on the name.
...
class GridSearchConfig(BaseModel):
    ...
    def total_experiments(self) -> int:
        return len(self.chunking_configs) * len(self.embedding_models) * len(self.retrieval_methods)
