...
|-------|-------------|--------|
| 1 | Foundation -- Config, DB models (SQLAlchemy ORM for TAWOS schema), IssueService, convert_tawos.py | done |
| 2 | ChromaDB vector store, BM25 keyword search, hybrid search with score fusion, seed_vectors.py | done |
| 3 | LLM-backed query parser -- intent detection, entity extraction (regex + LLM), few-shot prompt | done |
| 4 | 5 CrewAI agents (Retrieval, Context, Suggestion, SprintPlanner, Documentation), crew orchestration | done |
| 5 | IssueWriter with simulation mode, pending_operations table, execute/discard workflow, bulk ops | done |
| 6 | Analytics service -- suggestion logging, feedback recording, acceptance rates, quality metrics | done |
| 7 | FastAPI with 33 endpoints across 8 routers (core, issues, context, suggestions, sprint, write, analytics, docs) | done |
| 8 | Typer CLI -- sync, search, query, context, suggest, plan-sprint, velocity, release-notes, chat, stats | done |
| 9 | Evaluation suite -- retrieval accuracy (Recall@5, hybrid vs semantic), suggestion quality | done |
| 10 | README, .env.example, ruff/pytest quality gates, demo video script, final cleanup | done |
